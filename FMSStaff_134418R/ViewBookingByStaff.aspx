<%@ Page Title="" Language="C#" MasterPageFile="~/FMS_Master.master" AutoEventWireup="true" CodeFile="ViewBookingByStaff.aspx.cs" Inherits="ViewBookingByStaff" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
<h1>View booking by staff name</h1>
Staff Name:

    <asp:DropDownList ID="staffDropDownList" runat="server" AutoPostBack="True" 
        DataSourceID="StaffSqlDataSource" DataTextField="StaffName" 
    ...
    <asp:SqlDataSource ID="BookingSqlDataSource" runat="server" 
        ConnectionString="<%$ ConnectionStrings:FacilityDBConnectionString %>" 
        SelectCommand="SELECT Booking.StaffId, Booking.Location, Booking.BookingDate, Booking.Purpose, StartTime.StartTime, EndTime.EndTime, Staff.StaffName, Resource.Description FROM Booking INNER JOIN Staff ON Booking.StaffId = Staff.StaffId INNER JOIN Resource ON Booking.Location = Resource.Location INNER JOIN StartTime ON Booking.StartTimeId = StartTime.id INNER JOIN EndTime ON Booking.EndTimeId = EndTime.id WHERE (Booking.StaffId = @staffId)">
        <SelectParameters>
            <asp:ControlParameter ControlID="staffDropDownList" Name="StaffId" 
